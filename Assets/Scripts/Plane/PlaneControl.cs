...
public partial class PlaneControl : MonoBehaviour
{
    ...
    void Update()
    {
        if (!isFlipping)
        {
            ...
            // Reset is now handled in HandleFlip completion.
        }
        else
        {
            HandleFlip();
    ...
    void ApplyFlightForces(Quaternion rotation)
    {
        ...
        // Bolt: Optimized - derive vectors from the rotation already fetched in FixedUpdate
        // to eliminate multiple native property calls (transform.forward, transform.up).
