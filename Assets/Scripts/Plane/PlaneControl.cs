...
public partial class PlaneControl : MonoBehaviour
{
    ...
    void Update()
    {
        if (!isFlipping)
        {
            AirControl();
            HandleThruster();
            // Bolt: Optimized - removed redundant ControlPlaneEffects() call; ThrusterBoost coroutine manages effect states.
            HandleAutoBalance();
    ...
    void CheckGroundBounds()
    {
        ...
        // Bolt: Optimized - use cachedGroundBounds to avoid the expensive Collider.bounds property access (native call)
